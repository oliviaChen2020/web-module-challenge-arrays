...
function addFlavors(array,flavor){
    return array.unshift(flavor);
}
     addFlavors(originalFlavors,"Mango");
...
function copy(newArray,flavors){
...
function filterByWord(array,string){
    ...
    console.log(filterByWord(originalFlavors,"Fudge"));
...
function getAverageWordLength(array){
    ...
    for(let i=0; i<array.length; i++){
        totalWordCount += array[i].split(" ").length;
    }
    return Math.round(totalWordCount/array.length);
}
...
function getRandomFlavors(array1,array2,array3,array4){
    let allFlavors=[...array1, ...array2, ...array3, ...array4];
    let newFlavorList=[];
    for(let i=0; i<31; i++ ) {
        let pickedFlavor=allFlavors[Math.floor(Math.random()*allFlavors.length)];
        newFlavorList.push(pickedFlavor);
    }
